 ...
 * 
 * Updates the estimasi_tanggal_perekaman for a duplicate operator record.
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!token.startsWith('eyJ')) {
      console.error('[DuplicateOperatorUpdateDate] Invalid token format detected');
      return NextResponse.json(
    ...
    try {
      ...
      console.error('[DuplicateOperatorUpdateDate] Failed to decode JWT:', e);
      return NextResponse.json(
    ...
    if (!['admin', 'superuser'].includes(userRole)) {
      console.error('[DuplicateOperatorUpdateDate] Insufficient permissions - role:', userRole);
      return NextResponse.json(
    ...
    const response = await fetch(
      `${goBackendUrl}/data-rekam/duplicate-operator/${id}/update-date`,
      {
        method: 'PATCH',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[DuplicateOperatorUpdateDate] Go backend error:', {
    ...
    console.error('[DuplicateOperatorUpdateDate] API error:', error);
    return NextResponse.json(
