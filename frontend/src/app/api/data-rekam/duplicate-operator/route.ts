 ...
 * 
 * Creates or updates a duplicate operator record using Supabase.
 * This is a secure API route that:
 ...
 * - nik_duplicate: NIK of the duplicate person
 * - nama_duplicate: Name of the duplicate person
 * - nik_operator: NIK of the operator
 * - nama_operator: Name of the operator
 * - nik_pengaju: NIK of the requestor
 * - nama_pengaju: Name of the requestor
 * - tanggal_perekaman: Date of recording (YYYY-MM-DD)
  ...
  try {
    ...
    for (const field of requiredFields) {
      if (!body[field] || String(body[field]).trim() === "") {
        console.error(
          `[duplicate-operator-api] Missing or empty required field: ${field}`
        );
        return NextResponse.json(
    ...
    if (!supabaseUrl || !supabaseServiceRoleKey) {
      console.error("[duplicate-operator-api] Missing Supabase configuration");
      return NextResponse.json(
    ...
    if (body.id) {
      ...
      if (error) {
        console.error("[duplicate-operator-api] Update error:", error);
        return NextResponse.json(
      ...
      if (error) {
        console.error("[duplicate-operator-api] Insert error details:", {
 ...
 * 
 * Deletes a duplicate operator record by ID.
 * 
 * Required:
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const authHeader = request.headers.get('authorization');
    if (!authHeader) {
      console.error('[duplicate-operator-api] Missing authorization header');
      return NextResponse.json(
        { success: false, error: 'Unauthorized: Missing Bearer token' },
        { status: 401 }
      );
    ...
    if (tokenParts.length !== 2 || tokenParts[0] !== 'Bearer') {
      console.error('[duplicate-operator-api] Invalid authorization format');
      return NextResponse.json(
    ...
    try {
      ...
      console.error('[duplicate-operator-api] Failed to parse request body');
      return NextResponse.json(
    ...
    const { id } = body;
    if (!id) {
      console.error('[duplicate-operator-api] Missing record ID');
      return NextResponse.json(
    ...
    if (error) {
      console.error('[duplicate-operator-api] Delete error details:', {
        message: error.message,
        code: (error as any).code,
        details: (error as any).details,
        hint: (error as any).hint,
