 ...
 * 
 * Rejects a pending user registration with an optional reason.
 * This is a secure API route that:
 * 1. Validates user authentication via their token
 * 2. Calls the Go backend /admin/reject-user endpoint
 * 3. Returns success/failure response
 * 
 * Only accessible to admin users (verified by Go backend)
 * 
 * Request body: { pending_user_id: string, rejection_reason?: string }
 */
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(
            `${goBackendUrl}/admin/reject-user`,
            {
                method: 'POST',
                headers: {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                {
                    success: false,
                    error: errorData.error || 'Failed to reject user'
                },
                { status: response.status }
            );
        }
        ...
        console.error('Error in reject-user route:', error);
        return NextResponse.json(
            {
                success: false,
