...
export function DocumentTracker({
  ...
  const safeRequiredDocs = requiredDocuments || [];
  const safeUploadedDocs = uploadedDocuments || [];
  const safeVerifiedDocs = verifiedDocuments || [];

  if (safeRequiredDocs.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        ...
        <p className="text-sm text-gray-500">
          Tidak ada dokumen yang diperlukan untuk tahap ini
        </p>
      </div>
    );
  }
  ...
  const documentStatuses: DocumentStatus[] = safeRequiredDocs.map(doc => ({
    name: doc.name,
    required: doc.required,
    uploaded: safeUploadedDocs.includes(doc.name),
    verified: safeVerifiedDocs.includes(doc.name),
  }));
