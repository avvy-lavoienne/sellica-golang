...
export class OptimizedAIOrchestrator {
  ...
  private constructor() {
    ...
    const { UpstashCacheServiceSingleton } = require('../../backend-utilities/cache/cache/UpstashCacheServiceFactory');
  ...
  private async getServiceInstance(serviceType: AIServiceType): Promise<any> {
    ...
    switch (serviceType) {
      case 'simple':
        const { SimpleResponseService } = await import('../../../src/services/chatbot/simpleResponseService');
        serviceInstance = new SimpleResponseService();
        break;
      case 'tensorflow':
        const { aiServiceTensorFlow } = await import('../chatbot/core/aiServiceTensorFlow');
      ...
      case 'enhanced':
        ...
        const { IntelligenceLayer } = await import('../../../src/services/core/IntelligenceLayer');
        const { ServiceContainer } = await import('../../../src/services/core/ServiceContainer');
        const container = ServiceContainer.getInstance();
        serviceInstance = new IntelligenceLayer(container);
        break;
      case 'groq':
        const { groqResponseEnhancer } = await import('../chatbot/core/groqResponseEnhancer');
