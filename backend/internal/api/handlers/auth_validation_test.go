...
func TestValidateNIK(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			shouldFail:    true,
			expectedError: "NIK harus terdiri dari 16 angka", // Fails length check first (length with space is 17)
		},
	}
...
func TestValidateNIP(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			shouldFail:    true,
			expectedError: "NIP harus terdiri dari 18 angka jika disediakan", // Fails length check first (length with space is 19)
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateNIP(tt.nip)
