...
// NewRAGPerformanceMonitor creates a new performance monitor with optional config
func NewRAGPerformanceMonitor() *RAGPerformanceMonitor {
	rpm := &RAGPerformanceMonitor{
		...
		maxIndexingTime:      500 * time.Millisecond, // Increased from 100ms to 500ms - realistic for document indexing
		logThresholdWarnings: false,                  // Disable by default to reduce startup noise
	}
	return rpm
}

// NewRAGPerformanceMonitorWithConfig creates a new performance monitor with custom configuration
func NewRAGPerformanceMonitorWithConfig(maxEmbeddingTime, maxSearchTime, maxIndexingTime time.Duration, logWarnings bool) *RAGPerformanceMonitor {
	return &RAGPerformanceMonitor{
		embeddingTimes:       make([]time.Duration, 0),
		searchTimes:          make([]time.Duration, 0),
		indexingTimes:        make([]time.Duration, 0),
		errorCounts:          make(map[string]int64),
		maxEmbeddingTime:     maxEmbeddingTime,
		maxSearchTime:        maxSearchTime,
		maxIndexingTime:      maxIndexingTime,
		logThresholdWarnings: logWarnings,
	}
}
...
func (rpm *RAGPerformanceMonitor) RecordIndexingTime(duration time.Duration, documentID ...string) {
	...
	// Check performance threshold - only log if explicitly enabled via config
	// This prevents noisy logs during startup when processing many documents
